...
(function( $ ) {
		...
		*/
        diyUpload:function( opt, serverCallBack ) {
			...
			webUploader.on('uploadSuccess',function( file, response ){
				...
				$fileBox.attr('data-id', timeId);
				response.info.timeId = timeId;
	...
	// 取消事件 - 服务器
	function removeImg($li)
	{
		// 删除物理图片
		// 这里注意的是 $diyCancel绑定事件执行完后 才执行的回调diyCancel 所以diyCancel不必要弹出任何提示
		if (isExitsFunction('diyCancel')) {
			diyCancel($li);
        }
	}
	...
	function createBox( $fileInput, file, webUploader ) {
		...
		if ( $parentFileBox.find('.diyButton').length <= 0 ) {
			...
			//绑定开始上传按钮;
			//$startButton.on('click',uploadStart);
			//$("#submitData").on('click',uploadStart);
			$(".webuploader-element-invisible").live("change",function(){
				uploadStart();
			});
	...
	$(".fileBoxUl li").live({
		mouseenter:function(){
			$(this).children(".diyCancel").show();
		},
		mouseleave:function(){
			$(this).children(".diyCancel").hide();
		}
	})
})( jQuery );